classdef pose < handle
    ...
    methods(Access = public)
       ...
       function newPose = updatePoseOdom(obj, V, w, dt)
           currX = obj.poseVec(1);
           currY = obj.poseVec(2);
           currTh = obj.poseVec(3);
           
           % update theta
           newTh = double( (currTh + w*dt) );
            
            % get x,y vector from theta
            dx = double(V*cos(newTh));
            dy = double(V*sin(newTh));
            % update x and y position
            newX = currX + dx*dt;
            newY = currY + dy*dt;
            
            newPose = pose(newX, newY, newTh);
       end
