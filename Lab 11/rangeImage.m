classdef rangeImage < handle
    ...
    methods(Access = public)
        ...
        function [x y th] = bestLineCandidate(obj,palletLen,firstPass)
            [maxError n th x y len] = obj.findLineCandidate(1);
            % check every possible midpoint
            for mid = 2:length(obj.rArray)
                % check this candidate
                [e n t midX midY len] = obj.findLineCandidate(mid);
                % find line with greatest number of pixels
                if (e < maxError) && (len > palletLen * .6) && (len < palletLen * 1.2);
                    maxError = e;
                    % get th from sensor
                    th = t;     
        ...
        function [err num th midX midY len] = findLineCandidate(obj,middle)
            ...
            % check set of points within range
%            if(length(rArray) < maxScanIndex)  
                for i = 1:length(rArray)
                    % get distance of this pixel from mid
                    dist(i) = ((xArray(i)-midX)^2.0 + (yArray(i)-midY)^2.0)^0.5;             
                    % if out of range, add index to list of indices to be removed
                    if(dist(i) > maxLineLength) remove(length(remove)+1) = i; end              
                    % square distance after done using it
                    dist(i) = dist(i)^2;
                end
                dist(remove) = [];    
%             else
%                 for j = 1:maxScanIndex %length(rArray)
%                     % check ranges above mid
%                     i = obj.indexAdd(middle,j-maxScanIndex/2);
%                     % get distance of this pixel from mid
%                     dist(j) = ((xArray(i)-midX)^2.0 + (yArray(i)-midY)^2.0)^0.5;             
%                     % if out of range, add index to list of indices to be removed
%                     if(dist(j) > maxLineLength) remove(length(remove)+1) = i; end              
%                     % square distance after done using it
%                     dist(j) = dist(j)^2;                    
%                 end
%             end
%             
            ...
            % get length of line
            len = ((xArray(end)-xArray(1))^2+(yArray(end)-yArray(1))^2)^0.5;
