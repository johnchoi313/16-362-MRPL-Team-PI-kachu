classdef cubicSpiral < handle
            ...
            for a = -aMax:aMax/numA:aMax
                for b = -bMax:bMax/numB:bMax
                    ...
                    for i=1:clothSamples
                        % Compute the curve. 
                        % Break out of this loop, and then 
    ...
    methods(Access = public)
        ...
        function plotData(obj,xArray,yArray,simXOffset,simYOffset,simThOffset)
            % keep plot on 
            hold on
            % get simulated x and y plots
            simXArray = obj.poseArray(1,:);
            simYArray = obj.poseArray(2,:);
            % modify sim arrays with initial offset.
            simXArray = simXArray + simXOffset;
            simYArray = simYArray + simYOffset;
            % plot the information (meters)
            plot(simXArray,simYArray,xArray,yArray),...
            % plot information
            axis([-1 1 -1 1]),...
            title('X/Y Position'),...
            xlabel('Y (meters)'),...
